...
class MiniCourt():
    def __init__(self,frame):
        self.drawing_rectangle_width = 250 #pixels
    ...
    def set_canvas_background_box_position(self,frame): # white background
        frame= frame.copy()

        self.end_x = frame.shape[1] - self.buffer # width 
        self.end_y = self.buffer + self.drawing_rectangle_height # height
    ...
    def draw_background_rectangle(self,frame):
        ...
        out[mask] = cv2.addWeighted(frame, alpha, shapes, 1 - alpha, 0)[mask] # combine the two images, with alpha and beta weights
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self,player_boxes, ball_boxes, original_court_key_points ):
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            # Measure distance from Ball and Player and returns key of minimum bounding box
            ...
            for player_id, bbox in player_bbox.items():
                ...
                # Detect height of box in pixels
                bboxes_heights_in_pixels = [get_height_of_bbox(player_boxes[i][player_id]) for i in range (frame_index_min,frame_index_max)] # list of bbox heights
                max_player_height_in_pixels = max(bboxes_heights_in_pixels) # obtain max height
                ...
                if closest_player_id_to_ball == player_id:
                    ...
                    output_ball_boxes.append({1:mini_court_player_position}) # only 1 ball
            output_player_boxes.append(output_player_bboxes_dict)

        return output_player_boxes , output_ball_boxes
     
    def draw_points_on_mini_court(self,frames,postions, color=(0,255,0)):
        for frame_num, frame in enumerate(frames): #loop over frames
            for _, position in postions[frame_num].items():
                x,y = position
                x= int(x) # make positions integers
                y= int(y)
